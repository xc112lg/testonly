...
complete() {
  ...
  echo "   a) Back up your device's current boot/product/system_ext/vendor:"
  echo "      $0 --backup-only"
  ...
  echo "      against your device's CURRENT product/system_ext/vendor, not"
  echo "      the new ones, so it won't catch every possible issue. There is"
  echo "      no equivalent temporary-test mechanism for product/system_ext/"
  echo "      vendor themselves (unlike boot.img, they're mounted filesystems"
  echo "      checked at early boot, not something fastboot can boot from RAM)."
  echo "      DSU does not apply here — it only swaps the system partition,"
  echo "      which this port doesn't build or touch at all."
  echo ""
