...
package_flashable_zip() {
  ...
  # NOTE: recovery's bundled `unzip` (TWRP/OrangeFox, etc.) is often a
  # minimal implementation that doesn't reliably support ZIP64 extensions
  # needed for entries approaching/exceeding the classic 4GB-per-entry ZIP
  # limit — even when the zip itself is written correctly. system.img in
  # particular (target's full stock system + donor's merged system_ext) can
  # easily be large enough to hit this. Any image over CHUNK_MB gets split
  # into fixed-size chunks before zipping; each chunk stays comfortably
  # under the limit. The device-side installer reassembles by dd'ing each
  # chunk directly to its correct offset (via seek=) rather than needing to
  # stage a full reassembled copy on the recovery's limited /tmp.
  local CHUNK_MB=900
  for img in boot product system vendor; do
    local src="$WORK_DIR/final_images/${img}.img"
    local size_mb
    size_mb=$(( $(stat -c%s "$src") / 1024 / 1024 ))
    if [ "$size_mb" -gt "$CHUNK_MB" ]; then
      log "  ${img}.img is ${size_mb}MB — splitting into ${CHUNK_MB}MB chunks for the zip"
      split -b "${CHUNK_MB}M" -d -a 3 "$src" "$ZIP_DIR/images/${img}.img.part"
      [ -n "$(ls -A "$ZIP_DIR/images/${img}.img.part"* 2>/dev/null)" ] || error "Failed to split ${img}.img into chunks"
    else
      cp "$src" "$ZIP_DIR/images/"
    fi
  done
...
flash_image() {
  ...
  # Try as a single whole-file entry first (small partitions aren't split).
  rm -rf "$TMPDIR/extract" "$tmp_img"
  mkdir -p "$TMPDIR/extract"
  unzip -o "$ZIPFILE" "images/${part}.img" -d "$TMPDIR/extract" >/dev/null 2>&1
  if [ $? -eq 0 ] && [ -f "$TMPDIR/extract/images/${part}.img" ]; then
  ...
  # Fall back to chunked parts (large images, e.g. system.img, are split
  # PC-side into ${CHUNK_MB}MB pieces to stay under the ~4GB per-entry ZIP
  # limit some minimal recovery unzip implementations can't handle). Each
  # chunk is extracted, dd'd directly to its correct byte offset via
  # seek=, then deleted — never reassembling the full image on /tmp.
  ui_print "- Flashing $part -> $dev (chunked, ${CHUNK_MB}MB pieces)"
  local i=0 entry chunk_file wrote_any=0
  while :; do
    entry=$(printf "images/${part}.img.part%03d" "$i")
    rm -rf "$TMPDIR/extract"
    mkdir -p "$TMPDIR/extract"
    unzip -o "$ZIPFILE" "$entry" -d "$TMPDIR/extract" >/dev/null 2>&1
    chunk_file="$TMPDIR/extract/$entry"
    [ -f "$chunk_file" ] || { rm -rf "$TMPDIR/extract"; break; }

    dd if="$chunk_file" of="$dev" bs=1M seek=$((i * CHUNK_MB)) conv=notrunc 2>&1
    if [ $? -ne 0 ]; then
      ui_print "! dd failed writing chunk $i of $part"
      rm -rf "$TMPDIR/extract"
      return 1
    fi
    wrote_any=1
  ...
  if [ "$wrote_any" -eq 0 ]; then
    ui_print "! images/${part}.img not found in zip (whole-file or chunked), skipping"
    return 1
  fi
  return 0
}
...
exit 0
UPDATER_EOF
  sed -i "s/##CHUNK_MB##/${CHUNK_MB}/" "$ZIP_DIR/META-INF/com/google/android/update-binary"
